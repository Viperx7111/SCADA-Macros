...
' ============================================================================
' SCADA POINTS LIST GENERATOR  —  Rev 12
' ============================================================================
'
' CHANGES FROM Rev 11:
'   BUG FIXES
'     - RunCompareEngine: COMMENTS column was declared "Always ignored in compare"
'       but had no implementation; added ptsCommentsCol variable, FindHeader call,
'       and exclusion guard matching the existing AESO column pattern.
'
' CHANGES FROM Rev 10:
'   BUG FIXES
'     - CLR_CELL_RED comment corrected: RGB(255, 128, 128) not RGB(255, 150, 150)
'     - FindOrphanedPoints CleanExit: added Set wsOrphans = Nothing
'     - RunCompareEngine CleanExit: added Set wsCompare/dictExistingDevs/dictExpTagToDev = Nothing
'
'   PERFORMANCE
'     - FilterReviewedPoints: replaced cell-by-cell reads from Compare/Orphaned
'       sheets with single bulk Range.Value reads (fewer COM round-trips)
'     - FindOrphanedPoints: orphan highlights now batched via Union, applied in
'       one Interior.Color call instead of one per row
'     - UpdateCategoryFromSelection: per-row category write batched into a
'       1xN array per row instead of one cell write per column
'     - InsertMissingPoints: status bar in insertion loop throttled to every
'       STATUS_INTERVAL rows (same pattern as RunCompareEngine)
'     - ProgressBar: removed redundant CLng(Int(...)) double-conversion
'
'   DOCUMENTATION
'     - TAG_NAME_SEP: separator constraint documented
'     - RPL_TOKENS: token prefix naming convention explained
'     - BuildExpectedArray Case "X": explained why outArr is read instead of tmpData
'     - RunCompareEngine: blank-description skip business rule clarified
'     - GetLocalFolder: added step-by-step URL stripping example
'     - LoadContext column map: added conflict-resolution example
'
'   STREAMLINING
'     - GenerateCompletionReport: three identical selection-sort blocks replaced
'       with a shared SortDeviceList helper Sub
'
' ============================================================================
'
' PURPOSE:
...
' CONSTRAINT: this character must NOT appear in any IED NAME or TAG SUFFIX
' value, because TAG NAME is also used as a unique lookup key in the compare
' and orphan engines.  If either part already contains the separator, the key
' becomes ambiguous (e.g. "A_B" + "_" + "C" vs "A" + "_" + "B_C" both yield
' "A_B_C").  The underscore is the conventional SCADA tag separator; avoid
' using it in device names or point suffixes.
...
'
' ALGORITHM - step by step with a concrete example:
'
'   Suppose wb.Path = "https://d.docs.live.net/ABC123/Work/SCADA Macros"
'   and the local OneDrive root is "C:\Users\chris\OneDrive".
'
'   Step 1 - Strip protocol and hostname:
'     sRel = Mid("d.docs.live.net/ABC123/Work/SCADA Macros", first "/")
'          = "/ABC123/Work/SCADA Macros"
'     sRel = Replace(sRel, "/", "\")
'          = "\ABC123\Work\SCADA Macros"
'
'   Step 2 - For each OneDrive root, try appending sRel and progressively
'     stripping the leading path segment to absorb the OneDrive account CID
'     (ABC123 here) and any other URL-only prefixes:
'
'     j=0: sCandidate = "C:\Users\chris\OneDrive\ABC123\Work\SCADA Macros"
'          Dir(sCandidate) = "" (not found - ABC123 is a URL segment, not local)
'          nSlash = InStr(2, "\ABC123\Work\SCADA Macros", "\") -> 8
'          sTry = "\Work\SCADA Macros"
'
'     j=1: sCandidate = "C:\Users\chris\OneDrive\Work\SCADA Macros"
'          Dir(sCandidate) = "SCADA Macros" (found!) -> return this path
'
'   The inner loop tries up to 7 segment-stripping iterations, which is
'   enough to handle any realistic OneDrive URL prefix depth.
' ============================================================================
...
Public Sub FilterReviewedPoints()
    ...
    Dim bulkArr      As Variant   ' bulk column-B read from helper sheets
    Dim r            As Long
    Dim lastRow      As Long
    ...
    ' Bulk-read the entire column B from row 3 to the last used row in one
    ' Range.Value call, then scan the resulting array.  This replaces the
    ' original cell-by-cell loop and eliminates one COM round-trip per row.
    ...
    If Not wsTest Is Nothing Then
        ...
        If lastRow >= 3 Then
            bulkArr = wsTest.Range(wsTest.Cells(3, 2), wsTest.Cells(lastRow, 2)).Value
            If IsArray(bulkArr) Then
                For r = 1 To UBound(bulkArr, 1)
                    tagVal = SafeStrVal(bulkArr(r, 1))
                    If tagVal <> "" Then dictTags(tagVal) = 1
                Next r
            Else
                ' Single-cell result (only one data row)
                tagVal = SafeStrVal(bulkArr)
                If tagVal <> "" Then dictTags(tagVal) = 1
            End If
        End If
        Set wsTest = Nothing
    ...
    ' Same bulk-read approach as the Compare sheet above.
    On Error Resume Next
...
BwdNext:
    ...
    ' Throttle status bar updates to every STATUS_INTERVAL rows.
    ' Updating the status bar on every iteration forces a Windows UI event-loop
    ' cycle, which slows down tight loops significantly on large inserts.
    Const STATUS_INTERVAL As Long = 10

    ReDim rowData(1 To 1, 1 To numPtsCols)

    For i = 1 To insertCount
        If i Mod STATUS_INTERVAL = 1 Or i = insertCount Then
...
Public Sub UpdateCategoryFromSelection()
    ...
    Dim rowData()    As Variant   ' 1xN staging array for bulk row write
    ...
    ' Pre-allocate a 1-row staging array for the bulk write.
    ' Sized once here; reused for every selected row so VBA doesn't
    ' reallocate on each iteration.
    ReDim rowData(1 To 1, 1 To numPtsCols)
    ...
    For k = 0 To UBound(selRowKeys)
        ...
        ' First pass: read the existing row into rowData (to preserve any
        ' columns that are NOT sourced from the category table).
        ' Second pass: overwrite only the category-mapped positions.
        ' Then write the whole row back in a single bulk call.
        ' This replaces the original per-column wsPoints.Cells(...).Value = catVal
        ' loop (numPtsCols individual COM calls) with a single Range.Value write.
        Dim existRow As Variant
        existRow = wsPoints.Range( _
            wsPoints.Cells(shRow, dataFirstCol), _
            wsPoints.Cells(shRow, dataFirstCol + numPtsCols - 1)).Value
        ...
        For c = 1 To numPtsCols
            If catColMap(c) > 0 Then
                ...
                If Not IsError(catVal) Then
                    ...
                    rowUpdated = True
                Else
                    rowData(1, c) = existRow(1, c)  ' keep existing value
                End If
            Else
                rowData(1, c) = existRow(1, c)  ' keep existing value
            End If
        ...
        If rowUpdated Then
            wsPoints.Range( _
                wsPoints.Cells(shRow, dataFirstCol), _
                wsPoints.Cells(shRow, dataFirstCol + numPtsCols - 1)).Value = rowData
...
Public Sub FindOrphanedPoints()
    ...
    Dim r               As Long
    Dim lr              As Long
    Dim orphanUnion     As Range  ' accumulates orphan rows for batch highlighting
    ...
    For r = 1 To existRows
        ...
        If Len(reason) > 0 Then
            ' Accumulate orphan rows into a Union range for batch highlighting
            ' below.  Building the Union here (in the scan loop) and applying
            ' the colour once after the loop reduces COM calls from one per
            ' orphan to a single Interior.Color call regardless of count.
            Dim orphanRowRange As Range
            Set orphanRowRange = wsPoints.Range( _
                wsPoints.Cells(dataStartRow + r - 1, dataStartCol), _
                wsPoints.Cells(dataStartRow + r - 1, dataStartCol + numPtsCols - 1))
            If orphanUnion Is Nothing Then
                Set orphanUnion = orphanRowRange
            Else
                Set orphanUnion = Application.Union(orphanUnion, orphanRowRange)
            End If
...
NextOrphanRow:
    ...
    ' Apply all orphan highlights in a single Interior.Color call.
    If Not orphanUnion Is Nothing Then orphanUnion.Interior.Color = CLR_ORPHAN
...
Public Sub GenerateCompletionReport()
    ...
    Dim sortArr()           As Variant  ' 2D staging array used by SortDeviceList calls
...
NextDataRow:
    ...
    ' Sort all three device lists (completed, incomplete, zero-point) by
    ' IED AREA then IED NAME using the shared SortDeviceList helper.
    ' Each list is packed into a 2D Variant array, sorted, then unpacked.
    ' This eliminates three near-identical selection-sort blocks and keeps
    ' the sort algorithm in one place.

    ' -- Sort completed (cols: name, area, pts) --
    If completedCount > 1 Then
        ReDim sortArr(1 To completedCount, 1 To 3)
        For i = 1 To completedCount
            sortArr(i, 1) = completedNames(i)
            sortArr(i, 2) = completedAreas(i)
            sortArr(i, 3) = completedPts(i)
        Next i
        SortDeviceList sortArr, completedCount
        For i = 1 To completedCount
            completedNames(i) = CStr(sortArr(i, 1))
            completedAreas(i) = CStr(sortArr(i, 2))
            completedPts(i)   = CLng(sortArr(i, 3))
        Next i
    End If

    ' -- Sort incomplete (cols: name, area, filled, totals) --
    If incompletedCount > 1 Then
        ReDim sortArr(1 To incompletedCount, 1 To 4)
        For i = 1 To incompletedCount
            sortArr(i, 1) = incompleteNames(i)
            sortArr(i, 2) = incompleteAreas(i)
            sortArr(i, 3) = incompleteFilled(i)
            sortArr(i, 4) = incompleteTotals(i)
        Next i
        SortDeviceList sortArr, incompletedCount
        For i = 1 To incompletedCount
            incompleteNames(i)  = CStr(sortArr(i, 1))
            incompleteAreas(i)  = CStr(sortArr(i, 2))
            incompleteFilled(i) = CLng(sortArr(i, 3))
            incompleteTotals(i) = CLng(sortArr(i, 4))
        Next i
    End If

    ' -- Sort zero-point (cols: name, area) --
    If zeroCount > 1 Then
        ReDim sortArr(1 To zeroCount, 1 To 2)
        For i = 1 To zeroCount
            sortArr(i, 1) = zeroNames(i)
            sortArr(i, 2) = zeroAreas(i)
        Next i
        SortDeviceList sortArr, zeroCount
        For i = 1 To zeroCount
            zeroNames(i) = CStr(sortArr(i, 1))
            zeroAreas(i) = CStr(sortArr(i, 2))
        Next i
    End If
...
Private Sub RunCompareEngine(ByVal runMode As Long)
    ...
    ptsCommentsCol = FindHeader(ptsHeaders, numPtsCols, HDR_COMMENTS)
    ...
    For r = 1 To existRows
        ...
        For c = 1 To numPtsCols
            ...
            ' BUSINESS RULE - "Unmanaged" points (blank template description):
            ' When the TEMP TAG DESCRIPTION for a template point is blank, the
            ' point is treated as "user-managed" — the engineer fills in the
            ' description and any downstream columns (POINT CATEGORY, alarm
            ' settings, etc.) by hand.  The automation owns only the columns
            ' that come before DESCRIPTION (device attributes, template IDs,
            ' tag suffix, tag name).
            '
            ' Practical effect on the compare loop:
            '   - If blankDesc: skip POINT CATEGORY (it was never set by
            '     generation, so any value the user entered is authoritative).
            '   - When column c reaches DESCRIPTION itself and expVal is empty,
            '     Exit For cuts the loop short, skipping DESCRIPTION and all
            '     columns to the right in one step.  This handles both the
            '     DESCRIPTION column and the category/alarm columns that follow.
            If blankDesc And ptsCatCol > 0 And c = ptsCatCol Then GoTo NextCmpCol

            ' Always ignore AESO and COMMENTS columns - these are manually
            ' maintained in the Points List and are never generated from source tables.
            If ptsAesoReqCol  > 0 And c = ptsAesoReqCol  Then GoTo NextCmpCol
            If ptsAesoDescCol > 0 And c = ptsAesoDescCol Then GoTo NextCmpCol
            If ptsCommentsCol > 0 And c = ptsCommentsCol Then GoTo NextCmpCol

            ' When the template has no description for this point, Exit For here
            ' to skip DESCRIPTION and all remaining columns (which are all
            ' user-owned for this point).
...
CleanExit:
    ...
    Set dictNone = Nothing
    Set dictExpected = Nothing
    Set dictExistingDevs = Nothing
    Set dictExpTagToDev = Nothing
    Set loPoints = Nothing
    Set wsPoints = Nothing
...
Private Function LoadContext() As Object
    ...
    ' Resolution order: TAG_NAME (derived) > Device List > Template Table >
    ' Category Table > Aliases (which then re-check D/T/C in the same order).
    '
    ' WHY this order?  Device List is the most specific/authoritative source
    ' for a given point row.  Template Table adds point-level attributes.
    ' Category Table adds alarm-class settings.  Aliases handle naming
    ' mismatches.  If a header appears in multiple tables (e.g. "I/O TEMPLATE"
    ' exists in both DEVICE_LIST and TEMPLATE_TABLE), the Device List value
    ' wins because it is checked first.  This is intentional: the device row
    ' is the primary context for each generated output row.
    '
    ' Unmatched columns: colSource(c) stays "" and the column is left blank
    ' in the output.  This is silent and intentional for Points List columns
    ' that are purely user-entered (e.g. AESO REQUIRED, AESO DESCRIPTION).
    ...
    For c = 1 To numPtsCols
        ...
        ElseIf aliasDict.Exists(hdr) Then
            ' Alias found: resolve the canonical source-table header and
            ' apply the same D/T/C priority order as above.
            ...
            For col = 1 To numPtsCols
                Select Case colSource(col)
                    ...
                    Case "X"  ' Derived: TAG NAME = IED NAME + "_" + TAG SUFFIX
                        ' WHY we read the suffix from outArr rather than tmpData directly:
                        '
                        ' The TAG SUFFIX column in the Points List may have a different
                        ' header name than in the Template Table (e.g. "TAG SUFFIX" in
                        ' Points List vs "IED TAG SUFFIX" in Template Table).  The alias
                        ' system resolves this during column mapping, and the resolved
                        ' value was already written to outArr(outRow, ptsSuffixCol) by the
                        ' "T" case a few iterations earlier in this same For col loop.
                        '
                        ' Reading back from outArr guarantees we use exactly the same
                        ' string that will appear in the output sheet, regardless of
                        ' which alias or source column supplied it.  Reading from tmpData
                        ' directly would bypass the alias resolution and could read the
                        ' wrong column (or a blank column) if an alias is in effect.
                        tagSuffix = ""
...
' ----------------------------------------------------------------------------
' SortDeviceList - Selection sort a 2D Variant array by column 2 (IED AREA)
' then column 1 (IED NAME).  Any additional columns are swapped in parallel,
' so the sort works for lists with 2, 3, or 4 columns without modification.
'
' PARAMETERS:
'   data  - 1-based 2D Variant array (rows 1..count, cols 1..N)
'             col 1 = IED NAME  (primary sort key, secondary)
'             col 2 = IED AREA  (primary sort key, primary)
'             col 3+ = any extra payload columns swapped together with keys
'   count - number of rows to sort (data may be dimensioned larger)
'
' ALGORITHM: Selection sort O(n²) - appropriate for typical SCADA device
' counts (tens to low hundreds of devices).  For very large lists a
' more efficient algorithm would be warranted.
' ----------------------------------------------------------------------------
Private Sub SortDeviceList(ByRef data() As Variant, ByVal count As Long)
    Dim i     As Long
    Dim j     As Long
    Dim k     As Long
    Dim ncols As Long
    ...
    For i = 1 To count - 1
        For j = i + 1 To count
            ' Sort by area first; break ties by name (both ascending, A-Z)
            If data(i, 2) > data(j, 2) Or _
               (data(i, 2) = data(j, 2) And data(i, 1) > data(j, 1)) Then
                ' Swap all columns for rows i and j together
                For k = 1 To ncols
                    tmpV = data(i, k): data(i, k) = data(j, k): data(j, k) = tmpV
                Next k
            End If
        Next j
    Next i
...
Private Function ProgressBar(ByVal current As Long, ByVal total As Long) As String
    ...
    pct = current / total
    ' CLng rounds to nearest integer; for a 25-block bar the rounding
    ' difference vs. flooring is visually imperceptible.
    filled = CLng(pct * BAR_WIDTH)
